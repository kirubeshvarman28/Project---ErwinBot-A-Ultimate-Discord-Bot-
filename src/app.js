...
class ErwinBot extends Client {
  ...
  startWebServer() {
    ...
    app.get('/', (req, res) => {
      res.status(200).json({ 
        message: 'ErwinBot System Online',
        version: '2.0.0',
        timestamp: new Date().toISOString()
      });
    });
  ...
  setupCronJobs() {
    ...
    cron.schedule('* * * * *', () => {
        checkGiveaways(this);
        TimedRoleService.checkExpiredRemovals(this);
    });
